...
const flipBackground = () => {
  backgroundflipCss.innerHTML = `
    .grid-item::before {
        transform: scale(${state.backgroundHorizontallyFlipped ? "-1," : "1,"}${
    state.backgroundVerticallyFlipped ? "-1" : "1"
  });
    }`;
};
...
const resetBackgroundPositions = () => {
  ...
  state.grid.forEach((r, row) => {
    r.forEach((tile, col) => {
      tile.style.backgroundPosition = `${bgPositions[tile.textContent].y}% ${
        bgPositions[tile.textContent].x
      }%`;
    });
  });
};
...
const toggleBorders = () => {
  ...
  if (borderCss.innerHTML === "") {
    ...
    borderCss.innerHTML = ".grid-item::before { padding: 1px; }";
    borderCss.innerHTML +=
      ".grid-item { border: 1px solid white; padding: 1px }";
